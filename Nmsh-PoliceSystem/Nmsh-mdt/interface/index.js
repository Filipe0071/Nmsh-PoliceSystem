...
function getFormattedDate(date, prefomattedDate = false, hideYear = false) {
    ...
    if (prefomattedDate) {
        return `${prefomattedDate} في ${hours}:${minutes}`;
    }

    if (hideYear) {
        return `${day}. ${month} في ${hours}:${minutes}`;
    }
...
function timeAgo(dateParam) {
    ...
    if (seconds < 5) {
        ...
        return `${seconds} منذ ثواني`;
    } else if (seconds < 90) {
        return 'منذ دقيقة';
    } else if (minutes < 60) {
        return `${minutes} دقائق مضت`;
    } else if (isToday) {
        return getFormattedDate(date, 'اليوم');
    } else if (isYesterday) {
        return getFormattedDate(date, 'أمس');
    } else if (isThisYear) {
...
$(document).ready(() => {
    ...
    $(".profile-items").on("click", ".profile-item", function () {
        let id = $(this).data("id");
        $.post('https://Nmsh-mdt/getProfileData', JSON.stringify({
            id: id
        }));
    })
    ...
    $(".bulletin-items-continer").on("keydown", ".bulletin-item", function (e) {
        if (e.keyCode === 13) {
            ...
            let time = new Date()
            $.post('https://Nmsh-mdt/newBulletin', JSON.stringify({
                title: title,
                info: info,
                time: time.getTime()
            }));
    ...
    $(".contextmenu").on("click", ".remove-bulletin", function () {
        ...
        $.post('https://Nmsh-mdt/deleteBulletin', JSON.stringify({
            id: BulletinId,
            time: time.getTime()
        }));
        ...
        if (e.keyCode === 13) {
            ...
            if (URL !== "") {
                ...
                setTimeout(() => {
                    ...
                    $.post('https://Nmsh-mdt/addGalleryImg', JSON.stringify({
                        cid: cid,
                        URL: URL
                    }));
                }, 250);
    ...
    $(".manage-profile-save").click(function () {
        if (canSaveProfile == true) {
            ...
            setTimeout(() => {
                ...
                $.post('https://Nmsh-mdt/saveProfile', JSON.stringify({
                    pfp: pfp,
    ...
    $(".manage-incidents-title-holder").on("click", ".manage-incidents-save", function () {
        if (canSaveProfile == true) {
            ...
            $.post('https://Nmsh-mdt/saveIncident', JSON.stringify({
                ID: dbid,
                title: title,
            ...
            setTimeout(() => {
                if (canRefreshIncidents == true) {
                    ...
                    setTimeout(() => {
                        $(".incidents-search-refresh").empty();
                        $(".incidents-search-refresh").html("تحديث");
                        canRefreshIncidents = true
                        $.post('https://Nmsh-mdt/getAllIncidents', JSON.stringify({}));
                    }, 1500);
                }
            }, 1000);
    ...
    $(".manage-incidents-title-holder").on("click", ".manage-incidents-create", function () {
        if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
            $(".manage-incidents-reports-content").val("غرفة العناية المركزة #: [ # ]\n\nرقم التقرير: [ رقم التقرير ]\n\nالوقت: [ التاريخ و الوقت هنا ]\n\nجراحة: [نعم/لا]\n\الإصابات/الأمراض:\n - [ أدخل قائمة الإصابات هنا ]\n\n\nحضور إضافي:\n - [ ضع قائمة بأي موظفين آخرين هنا ]\n\n\n🧑‍🤝‍🧑 اتصالات الطوارئ الإضافية:\n - [ الإسم والرقم ]\n\n\nملاحظات:\n[ملاحظات إضافية هنا]");
            $("#manage-incidents-title-input").val("الإسم - " + $(".date").html());
            $(".manage-incidents-editing-title").html("تقوم حاليًا بإنشاء تقرير تسجيل جديد");
            $(".manage-incidents-title-holder").empty();
            $(".manage-incidents-title-holder").prepend(
                `
                <div class="manage-incidents-title">إدارة تسجيل الوصول في وحدة العناية المركزة</div>
            ...
            $(".manage-incidents-reports-content").val("📝 ملخص:\n\n[أدخل ملخص التقرير هنا]\n\n🧍 رهينة: [إسم الرهينة]\n\n🔪 الأسلحة / الأشياء المصادرة:\n\n· [إدراج قائمة هنا]\n\n-----\n💸 المخالفة:\n⌚ العقوبة:\n-----");
            $("#manage-incidents-title-input").val("الإسم - Charge - " + $(".date").html());
            $(".manage-incidents-editing-title").html("أنت تقوم حاليًا بإنشاء قضية جديدة");
            $(".manage-incidents-title-holder").empty();
            $(".manage-incidents-title-holder").prepend(
                `
                <div class="manage-incidents-title">ادارة القضية</div>
                <div class="manage-incidents-create"> <span class="fas fa-plus" style="margin-top: 3.5px;"></span></div>
                <div class="manage-incidents-save"><span class="fas fa-save" style="margin-top: 3.5px;"></span></div>
                `
            );
    ...
    $(".contextmenu").on("click", ".search-vehicle", function () {
       ...
       setTimeout(() => {
           ...
           setTimeout(() => {
               ...
               $.post('https://Nmsh-mdt/searchVehicles', JSON.stringify({
                   name: plate.toString()
               }));
               $(".dmv-items").empty();
                $('.dmv-items').prepend(`<div class="profile-loader"></div>`);
               setTimeout(() => {
                   $.post('https://Nmsh-mdt/getVehicleData', JSON.stringify({
    ...
    $(".contextmenu").on("click", ".search-weapon", function () {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                $.post('https://Nmsh-mdt/searchWeapon', JSON.stringify({
                    name: plate.toString()
                }));
                $(".weapons-items").empty();
                $('.weapons-items').prepend(`<div class="profile-loader"></div>`);
                setTimeout(() => {
                    $.post('https://Nmsh-mdt/getWeaponData', JSON.stringify({
    ...
    $(".contextmenu").on("click", ".revoke-licence", function () {
        $.post('https://Nmsh-mdt/updateLicence', JSON.stringify({
    ...
    $('#profile-search-input').keydown(function (e) {
        if (e.keyCode === 13 && canSearchForProfiles == true) {
            ...
            if (name !== "") {
                ...
                $.post('https://Nmsh-mdt/searchProfiles', JSON.stringify({
                    name: name
                }));
                $(".profile-items").empty();
    ...
    $('#incidents-search-input').keydown(function (e) {
        if (e.keyCode === 13 && canSearchForProfiles == true) {
            ...
            if (incident !== "") {
                canSearchForProfiles = false
                $.post('https://Nmsh-mdt/searchIncidents', JSON.stringify({
                    incident: incident
                }));
    ...
    $('#dispatchmsg').keydown(function (e) {
        ...
        if (keyCode === 13 && !e.shiftKey) {
            e.preventDefault()
            const time = new Date()
            $.post('https://Nmsh-mdt/dispatchMessage', JSON.stringify({
                message: $(this).val(),
                time: time.getTime()
            }));
            $(this).val("")
    ...
    $(".incidents-items").on("click", ".incidents-item", function () {
        ...
        $.post('https://Nmsh-mdt/getIncidentData', JSON.stringify({
            id: id
        }));
    })
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            ...
            if (shouldClose == true) {
                $.post('https://Nmsh-mdt/escape', JSON.stringify({}));
            }
        }
    };
    ...
    $(".icidents-person-search-name-input").on("keydown", "", function (e) {
        if (e.keyCode === 13) {
            ...
            $.post('https://Nmsh-mdt/incidentSearchPerson', JSON.stringify({
                name: name
            }));
        }
    });
    ...
    $(".manage-bolos-title-holder").on("click", ".manage-bolos-save", function () {
        ...
        $.post('https://Nmsh-mdt/newBolo', JSON.stringify({
            existing: existing,
            id: id,
            title: title,
            plate: plate,
    ...
    $('#bolos-search-input').keydown(function (e) {
        if (e.keyCode === 13 && canSearchForProfiles == true) {
            ...
            if (searchVal !== "") {
                canSearchForProfiles = false
                $.post('https://Nmsh-mdt/searchBolos', JSON.stringify({
                    searchVal: searchVal
                }));
                $(".bolos-items").empty();
    ...
    $(".bolos-search-refresh").click(function () {
        if (canRefreshBolo == true) {
            ...
            setTimeout(() => {
                $(".bolos-search-refresh").empty();
                $(".bolos-search-refresh").html("تحديث");
                canRefreshBolo = true
                $.post('https://Nmsh-mdt/getAllBolos', JSON.stringify({}));
            }, 1500);
        }
    });
    ...
    $(".bolos-items").on("click", ".bolo-item", function () {
        ...
        canInputBoloOfficerTag = true
        let id = $(this).data("id");
        $.post('https://Nmsh-mdt/getBoloData', JSON.stringify({
            id: id
        }));
    })
    ...
    $(".contextmenu").on("click", ".bolo-delete", function () { 
        ...
        if ($(this).data("info") != 0) {
            if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
                ...
                $.post('https://Nmsh-mdt/deleteICU', JSON.stringify({
                id: $(this).data("info")
            }));
        }
        $(".bolos-items").find("[data-id='" + $(this).data("info") + "']").remove();
        $.post('https://Nmsh-mdt/deleteBolo', JSON.stringify({
            id: $(this).data("info"),
    ...
    $(".contextmenu").on("click", ".incident-delete", function () { 
        ...
        $.post('https://Nmsh-mdt/deleteIncident', JSON.stringify({
            id: $(this).data("info"),
            time: new Date().getTime()
        }));
    });
    ...
    $(".contextmenu").on("click", ".missing-delete", function () { 
        ...
        $.post('https://Nmsh-mdt/deleteMissing', JSON.stringify({
            id: $(this).data("info"),
            time: new Date().getTime()
        }));
    });
    ...
    $(".contextmenu").on("click", ".profile-missing", function () { 
        $.post('https://Nmsh-mdt/missingCitizen', JSON.stringify({
            cid: $(this).data("info"),
    ...
    $(".contextmenu").on("click", ".report-delete", function () { 
        ...
        $.post('https://Nmsh-mdt/deleteReport', JSON.stringify({
            id: $(this).data("info"),
            time: new Date().getTime()
        }));
    });
    ...
    $(".contextmenu").on("click", ".add-charge", function () { 
        ...
        $.post('https://Nmsh-mdt/getPenalCode', JSON.stringify({}));
    });
    ...
    $(".contextmenu").on("click", ".associated-incidents-remove-tag", function () { 
        ...
        if (incidentId != 0) {
            $.post('https://Nmsh-mdt/removeIncidentCriminal', JSON.stringify({
            ...
            if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
                $(".incidents-ghost-holder").prepend(
                    ...
                    <div class="associated-incidents-user-container" data-id="${$(this).data("cid")}">
                        ...
                        <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">الغرامة الموصى بها</div>
    ...
    $(".contextmenu").on("click", ".incidents-remove-normal-tag", function () { 
        ...
        let cid = $(".manage-profile-citizenid-input").val();
        if (cid) {
            $.post('https://Nmsh-mdt/removeProfileTag', JSON.stringify({
                cid: cid,
                text: $(this).data("info")
            }));
        }
    ...
    $(".reports-search-refresh").click(function () {
        if (canRefreshReports == true) {
            ...
            setTimeout(() => {
                $(".reports-search-refresh").empty();
                $(".reports-search-refresh").html("تحديث");
                canRefreshReports = true
                $.post('https://Nmsh-mdt/getAllReports', JSON.stringify({}));
            }, 1500);
        }
    });
    $(".dispatch-comms-refresh").click(function(){
        ...
        setTimeout(() => {
            $(".dispatch-comms-refresh").empty();
            $(".dispatch-comms-refresh").html("تحديث");
            canRefreshReports = true
            $.post('https://Nmsh-mdt/refreshDispatchMsgs', JSON.stringify({}));
        }, 1500);
    })
    $(".reports-items").on("click", ".reports-item", function () {
        ...
        let id = $(this).data("id");
        $.post('https://Nmsh-mdt/getReportData', JSON.stringify({
            id: id
        }));
    })
    ...
    $('#reports-search-input').keydown(function (e) {
        if (e.keyCode === 13 && canSearchForReports == true) {
            ...
            if (name !== "") {
                ...
                $.post('https://Nmsh-mdt/searchReports', JSON.stringify({
                    name: name
                }));
                $(".reports-items").empty();
    ...
    $(".manage-reports-title-holder").on("click", ".manage-reports-save", function () {
        ...
        $.post('https://Nmsh-mdt/newReport', JSON.stringify({
            existing: existing,
            id: id,
            title: title,
            type: type,
    ...
    $('#missing-search-input').keydown(function (e) {
        if (e.keyCode === 13) {
            ...
            if (name !== "") {
                $.post('https://Nmsh-mdt/searchMissing', JSON.stringify({
                    name: name
                }));
                $(".missing-items").empty();
    ...
    $(".missing-items").on("click", ".missing-item", function () {
        $.post('https://Nmsh-mdt/getMissingData', JSON.stringify({
            id: $(this).data("dbid")
        }));
    });
    ...
    $(".vehicle-information-title-holder").on("click", ".vehicle-information-save", function () {
        if (canSaveVehicle) {
            ...
            setTimeout(() => {
                ...
                $.post('https://Nmsh-mdt/saveVehicleInfo', JSON.stringify({
                    dbid: dbid,
                    plate: plate,
                    imageurl: imageurl,
                    notes: notes
    ...
    $(".weapon-information-title-holder").on("click", ".weapon-information-save", function () {
        if (canSaveVehicle) {
            ...
            setTimeout(() => {
                ...
                $.post('https://Nmsh-mdt/saveWeaponInfo', JSON.stringify({
                    dbid: dbid,
    ...
    $(".missing-information-title-holder").on("click", ".missing-information-save", function () {
        if (canSaveVehicle) {
            ...
            setTimeout(() => {
                ...
                $.post('https://Nmsh-mdt/saveMissingInfo', JSON.stringify({
                    id: dbid,
                    imageurl: imageurl,
                    notes: notes
                }));
    ...
    $(".contextmenu").on("click", ".mark-code-5", function () { 
        ...
        if ( tag.hasClass("red-tag")) {
            ...
            $.post('https://Nmsh-mdt/knownInformation', JSON.stringify({
    ...
    $(".contextmenu").on("click", ".toggle-duty", function () { 
        ...
        if (currentStatus == "10-8") {
            ...
            $.post('https://Nmsh-mdt/toggleDuty', JSON.stringify({
                cid: info,
                status: 0
            }));
    ...
    $(".contextmenu").on("click", ".set-waypoint", function () { 
        let info = $(this).data("info")
        $.post('https://Nmsh-mdt/setWaypointU', JSON.stringify({
            cid: info,
        }));
    })
    ...
    $(".contextmenu").on("click", ".set-waypoint", function () { 
        ...
        $.post('https://Nmsh-mdt/setWaypoint', JSON.stringify({
            callid: callId,
        }));
    })
    ...
    $(".contextmenu").on("click", ".call-attach", function () { 
        const callId = $(this).data("info")
        $.post('https://Nmsh-mdt/callAttach', JSON.stringify({
            callid: callId,
        }));
    })
    ...
    $(".contextmenu").on("click", ".call-detach", function () { 
        const callId = $(this).data("info")
        $.post('https://Nmsh-mdt/callDetach', JSON.stringify({
            callid: callId
        }));
    })
    ...
    $(".contextmenu").on("click", ".remove-call", function () { 
        ...
        $.post('https://Nmsh-mdt/removeCall', JSON.stringify({
            callid: callId
        }));
    })
    ...
    $(".contextmenu").on("click", ".attached-units", function () { 
        const callId = $(this).data("info")
        $.post('https://Nmsh-mdt/attachedUnits', JSON.stringify({
            callid: callId
        }));
    })
    ...
    $('#respondcalls').keydown(function (e) {
        ...
        if (keyCode === 13 && !e.shiftKey) {
            ...
            e.preventDefault();
            const time = new Date()
            $.post('https://Nmsh-mdt/sendCallResponse', JSON.stringify({
                message: $(this).val(),
                time: time.getTime(),
                callid: callid
            }));
    ...
    $(".contextmenu").on("click", ".respond-call", function () {
        const callId = $(this).data("info")
        $.post('https://Nmsh-mdt/getCallResponses', JSON.stringify({
            callid: callId
        }));
    ...
    $(".contextmenu").on("click", ".call-dispatch-detach", function () { 
        const cid = $(this).data("info")
        $.post('https://Nmsh-mdt/callDispatchDetach', JSON.stringify({
    ...
    $(".contextmenu").on("click", ".Set-Dispatch-Waypoint", function () { 
        const cid = $(this).data("info")
        $.post('https://Nmsh-mdt/setDispatchWaypoint', JSON.stringify({
            callid: $(".dispatch-attached-units-container").attr("id"),
            cid: cid
        }));
    })
    ...
    $(".jail-buttons").on("click", ".jail-submit", function () {
        ...
        if (time.length > 0) {
            ...
            $.post('https://Nmsh-mdt/jailPlayer', JSON.stringify({
                cid: cid,
                time: time
            }));
            $(".jail-inner-container").slideUp(500)
    ...
    $(".callsign-buttons").on("click", ".callsign-submit", function () {
        ...
        if (callsign.length > 2) {
            ...
            $.post('https://Nmsh-mdt/setCallsign', JSON.stringify({
                cid: editingcallsign,
    ...
    $(".active-calls-list").on("click", ".active-calls-item", function (e) {
        ...
        if (callId && draggedElement) {
            $.post('https://Nmsh-mdt/callDragAttach', JSON.stringify({
                callid: callId,
                cid: draggedElement
            }));
            draggedElement = 0
    ...
    function JobColors(sentJob) {
        ...
        if (sentJob) {
            if (sentJob == 'police') {
                ...
                $(".header-title").html("شرطة ولاية لوس سناتوس");
                $(".bolo-nav-item").html('<i class="fas fa-car"></i> بولوس');
                $(".bolos-search-title").html("بولوس");                
                $("#bolos-search-input").attr('placeholder','...البحث عن بولو');
                $(".manage-bolos-title").html("ادارة البولو");
                $(".manage-bolos-editing-title").html("You are currently creating a new BOLO");
                $(".boloplate-title").html("اللوحة");
                $(".boloowner-title").html("صاحب المركبة");
                $(".boloindividual-title").html("Individual");
                $("#boloplate").attr('placeholder','ضع رقم اللوحة هنا');
                $("#bolodetail").attr('placeholder','تفاصيل بولو هنا ...');
                $(".convictions-title").html("قناعات معروفة");   
                $("#boloowner").attr('placeholder','صاحب المركبة');
                $("#boloindividual").attr('placeholder','ضع invidivual هنا ...');
                $("#home-warrants-container").fadeIn(0)
                $("#home-reports-container").fadeOut(0)
                $(".quote-span").html("القانون فوق الجميع");
                ...
                $(".dispatch-title-ofsomesort").html("برقية")
                ...
                $("#reports-officeun-involved-tag-title").html("الضباط المشاركون")
                $(".roster-iframe").attr("src", PoliceRoster)    
                $(".incidents-nav-item").html('<i class="fas fa-stethoscope"></i> القضايا');
                $(".incidents-title").html("القضايا");
                $("#incidents-search-input").attr('placeholder', 'بحث عن حادث ...');
                $(".manage-incidents-reports-content").attr('placeholder', 'يظهر محتوى تقرير الحادث هنا ...');
                $(".manage-incidents-title").html("إدارة القضايا");
                $(".manage-incidents-editing-title").html("أنت تقوم حاليًا بإنشاء حادثة جديدة");
                $(".manage-incidents-officeun-title").html("الضباط المشاركون");
                $(".manage-incidents-evidence-title").html("الأدلة");
                $(".associated-incidents-tags-title").html("المجرمون المتورطون");
                $(".convictions-title").html("قناعات معروفة");   
                ...
                $(".header-title").html("وزارة الصحة");
                $(".quote-span").html("الصحة هي الأهم");
                $(".incidents-nav-item").html('<i class="fas fa-stethoscope"></i> وحدة العناية المركزة');
                $(".incidents-title").html("تسجيلات وحدة العناية المركزة");
                $("#incidents-search-input").attr('placeholder','البحث في التسجيلات ...');
                $(".manage-incidents-reports-content").attr('placeholder','ينتقل محتوى تسجيل الوصول هنا ...');
                $(".manage-incidents-title").html("إدارة تسجيل الوصول في وحدة العناية المركزة");
                $(".manage-incidents-editing-title").html("انت تقوم بإنشاء تسجيل وصول جديد لوحدة العناية المركزة");
                $(".manage-incidents-officeun-title").html("المسعفين المشاركون");
                $(".manage-incidents-evidence-title").html("المعرض");
                $(".associated-incidents-tags-title").html("المريض المتورط");
                $(".convictions-title").html("العلاجات");                
                ...
                $("#reports-officeun-involved-tag-title").html("المسعفون المشاركين")
                $(".dispatch-title-ofsomesort").html("محادثة وزارة الصحة")
                $(".dispatch-comms-container").fadeIn(0)
                $(".manage-profile-name-input-1").attr("readonly", true)
                $(".manage-profile-name-input-2").attr("readonly", true)
    ...
    window.addEventListener('message', function (event) {
        ...
        if (e.type == "show") {
            ...
            $(".manage-profile-editing-title").html("أنت الأن تقوم بتعديل " + table["firstname"] + " " + table["lastname"])
            ...
            $('.licenses-holder').prepend(
                `
                <div class="license-tag ${car} car" data-type="car">سيارة</div>
                <div class="license-tag ${bike} bike" data-type="bike">دراجة نارية</div>
                <div class="license-tag ${truck} truck" data-type="truck">شاحنة</div>
                <div class="license-tag ${weapon} weapon" data-type="weapon">سلاح</div>
                <div class="license-tag ${pilot} pilot" data-type="pilot">طيار</div>
            `);
            $(".tags-holder").empty();
            $(".vehs-holder").empty();
                    ...
                    <div style="display: flex; flex-direction: column;">
                        <div class="profile-item-title">${name}</div>
                            <div class="profile-tags">
                                <div class="profile-tag ${car}">سيارة</div>
                                <div class="profile-tag ${bike}">دراجة نارية</div>
                                <div class="profile-tag ${truck}">شاحنة</div>
                                <div class="profile-tag ${weapon}">سلاح</div>
                                <div class="profile-tag ${pilot}">طيار</div>
                            </div>
                        </div>
            ...
            if ((BodyDisplay) == "block") {
                ...
                $.post('https://Nmsh-mdt/dispatchNotif', JSON.stringify({
                    data: value,
                }));
            }
            $('.dispatch-items').scrollTop($('.dispatch-items')[0].scrollHeight)
                ...
                if (value.type == "ambulance") {
                    if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
                        $('.incidents-items').prepend(
                            ...
                            <div class="incidents-top-holder">
                                ...
                                <div class="incedent-report-name">تقرير وحدة العناية المركزة</div>
                            </div>
                            <div class="incidents-bottom-holder">
                                <div class="incedent-report-id">الأيدي: ${value.id}</div>
                    ...
                    if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602343483260958/sasp_badge.png') {
                        $('.incidents-items').prepend(
                            ...
                            <div class="incidents-top-holder">
                                <div class="incidents-item-title">${value.title}</div>
                                <div class="incedent-report-name">تقرير الحادثة</div>
                            </div>
                            <div class="incidents-bottom-holder">
                                <div class="incedent-report-id">الأيدي: ${value.id}</div>
                                <div class="incedent-report-time-ago">${value.author} - ${timeAgo(Number(value.time))}</div>
                ...
                $.each(penalcode, function (index, value) {
                    $.each(value, function (i, v) {
                        ...
                        <div style="display: flex; flex-direction: row; width: 100%; margin: auto; margin-bottom: 0vh; padding-top: 0.75vh;">
                            ...
                            <div class="offfense-item-months">${v.months} الشهور - $${v.fine} - 0 Points(s)</div>
                        </div>
                        `)
                    })
                })
            ...
            if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
                $(".manage-incidents-editing-title").html("تقوم حاليا بتعديل تقرير " + table["id"]);
            } else {
                $(".manage-incidents-editing-title").html("أنت حاليا تقوم بتعديل تقرير الحادث " + table["id"]);
            }
            ...
            if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
                ...
                $(".manage-incidents-title-holder").prepend(
                    `
                    <div class="manage-incidents-title">إدارة الحادث</div>
                    <div class="manage-incidents-create"> <span class="fas fa-plus" style="margin-top: 3.5px;"></span></div>
                    <div class="manage-incidents-save"><span class="fas fa-save" style="margin-top: 3.5px;"></span></div>
                    `
                );
                    ...
                    if (value.associated == 1) {
                        $(".incidents-ghost-holder").prepend(
                            `<div class="associated-incidents-user-container" data-id="${value.cid}">
                                ...
                                <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">الغرامة الموصى بها</div>
                                ...
                                <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">المخالفة</div>
                                <div class="associated-incidents-fine-input" data-id="${value.cid}" style="display:none;"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="أدخل غرامة هنا ..." value="0" class="fine-amount" data-id="${value.cid}" type="number"></div>
                            </div>`
                        );
                    } else {
                        $(".incidents-ghost-holder").prepend(
                            `<div class="associated-incidents-user-container" data-id="${value.cid}">
                                ...
                                <div class="manage-incidents-title-tag" data-id="${value.cid}">الغرامة الموصى بها</div>
                                ...
                                <div class="manage-incidents-title-tag" data-id="${value.cid}">المخالفة</div>
                                <div class="associated-incidents-fine-input" data-id="${value.cid}"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="أدخل الغرامة هنا ..." value="0" class="fine-amount" data-id="${value.cid}" type="number"></div>
                            </div>`
                        );
                    }
                } else {
                    if (value.associated == 1) {
                        $(".incidents-ghost-holder").prepend(
                            `<div class="associated-incidents-user-container" data-id="${value.cid}">
                                ...
                                <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">الغرامة الموصى بها</div>
                                <div class="associated-incidents-fine-input" data-id="${value.cid}" style="display:none;"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="0" disabled class="fine-recommended-amount" id="fine-recommended-amount" data-id="${value.cid}" type="number"></div>
                                <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">العقوبات الموصى بها</div>
                                ...
                                <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">المخالفه</div>
                                <div class="associated-incidents-fine-input" data-id="${value.cid}" style="display:none;"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="أدخل الغرامة هنا ..." value="0" class="fine-amount" data-id="${value.cid}" type="number"></div>
                                <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">العقوبة</div>
                                <div class="associated-incidents-sentence-input" data-id="${value.cid}" style="display:none;"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="أدخل الأشهر هنا ..." value="0" class="sentence-amount" data-id="${value.cid}" type="number"></div>
                            </div>`
                        );
                    } else {
                        $(".incidents-ghost-holder").prepend(
                            `<div class="associated-incidents-user-container" data-id="${value.cid}">
                                ...
                                <div class="manage-incidents-title-tag" data-id="${value.cid}">الغرامة الموصى بها</div>
                                <div class="associated-incidents-fine-input" data-id="${value.cid}"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="0" disabled class="fine-recommended-amount" id="fine-recommended-amount" data-id="${value.cid}" type="number"></div>
                                <div class="manage-incidents-title-tag" data-id="${value.cid}">العقوبات الموصى بها</div>
                                ...
                                <div class="manage-incidents-title-tag" data-id="${value.cid}">المخالفه</div>
                                <div class="associated-incidents-fine-input" data-id="${value.cid}"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="أدخل الغرامة هنا ..." value="0" class="fine-amount" data-id="${value.cid}" type="number"></div>
                                <div class="manage-incidents-title-tag" data-id="${value.cid}">العقوبة</div>
                                <div class="associated-incidents-sentence-input" data-id="${value.cid}"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="أدخل الأشهر هنا ..." value="0" class="sentence-amount" data-id="${value.cid}" type="number"></div>
                            </div>`
                        );
                    }
                }
                ...
                $(".icidents-person-search-holder").prepend(
                    ...
                    <div class="icidents-person-search-item" data-info="${name} (#${value.id})" data-cid="${value.id}" data-name="${name}">
                        ...
                        <div class="icidents-person-search-item-right">
                            <div class="icidents-person-search-item-right-cid-title">رقم الهوية</div>
                            <div class="icidents-person-search-item-right-cid-input"><span class="fas fa-id-card"></span> ${value.id}</div>
                            <div class="icidents-person-search-item-right-name-title">الإسم</div>
            ...
            let table = e.data
            $(".manage-bolos-editing-title").html("أنت تقوم حاليًا بتحرير BOLO " + table["id"]);
            
            if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
                $(".manage-bolos-editing-title").html("أنت تقوم بتحرير ICU Check-in " + table["id"]);
            }
                    ...
                    if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
                        $('.bolos-items').prepend(
                            ...
                            <div class="bolo-bottom-holder">
                                <div class="bolo-report-id">الأيدي: ${value.id}</div>
                ...
                <div class="missing-item" data-id="${value.id}" data-dbid="${value.id}" data-cid="${value.identifier}">
                    ...
                    <div style="display: flex; flex-direction: column;">
                        ...
                        <div class="missing-bottom-info">
                            <div class="missing-id">الأيدي: ${value.id} · اخر ظهور: ${timeAgo(Number(value.lastseen))}</div>
                        </div>
                    </div>
                </div>
                `)
            ...
            $(".manage-bolos-editing-title").html("أنت تقوم حاليًا بتحرير BOLO " + id);
            ...
            $(".manage-reports-editing-title").html("أنت حاليا تقوم بتحرير التقرير " + id);
                    ...
                    if ($(".badge-logo").attr('src') == 'https://cdn.discordapp.com/attachments/770324167894761522/912602342275301396/ems_badge.png') {
                        $('.reports-items').prepend(
                            ...
                            <div class="reports-top-holder">
                                ...
                                <div class="reports-report-name">${value.reporttype} تقرير</div>
                            </div>
                            <div class="reports-bottom-holder">
                                <div class="reports-report-id">الأيدي: ${value.id}</div>
            ...
            $(".manage-reports-editing-title").html("أنت حاليا تقوم بتحرير التقرير " + table["id"]);
                ...
                <div class="dmv-item" data-id="${value.id}" data-dbid="${value.dbid}" data-plate="${value.plate}">
                    ...
                    <div style="display: flex; flex-direction: column;">
                        <div class="dmv-item-title">${value.model}</div>
                            <div class="dmv-tags">
                                <div class="dmv-tag ${impound}">حجز</div>
                                <div class="dmv-tag ${bolo}">بولو</div>
                                <div class="dmv-tag ${stolen}">مسروقة</div>
                                <div class="dmv-tag ${codefive}">حالة 5</div>
                            </div>
                        </div>
                        <div class="dmv-bottom-info">
                            <div class="dmv-id">الأيدي: ${value.id} · المالك: ${value.owner}</div>
                        </div>
                    </div>
                </div>
                `)
                ...
                <div class="weapon-item" data-id="${value.id}" data-dbid="${value.id}" data-plate="${value.serialnumber}">
                    ...
                    <div style="display: flex; flex-direction: column;">
                        ...
                        <div class="weapon-bottom-info">
                            <div class="weapon-id">الأيدي: ${value.id} · المالك: ${value.owner}</div>
                        </div>
                    </div>
                </div>
                `)
            ...
            $('.vehicle-tags').append(`<div class="vehicle-tag ${impound}">حجز</div>`);
            $('.vehicle-tags').append(`<div class="vehicle-tag ${bolo}">بولو</div>`);
            $('.vehicle-tags').append(`<div class="vehicle-tag ${codefive} code5-tag">كود 5</div>`);
            $('.vehicle-tags').append(`<div class="vehicle-tag ${stolen} stolen-tag">مسروقة</div>`);
            $(".vehicle-info-imageurl-input").val(table["image"]);
            ...
            $.each(ems, function (index, value) {
                ...
                <div class="active-unit-item" data-id="${value.cid}">
                    <div class="unit-status ${statuscolor}">${status}</div>
                    <div class="unit-job active-info-job-ambulance">Ambulance</div>
                    <div class="unit-name">(${callsign}) ${value.name}</div>
                    <div class="unit-radio">${radio}</div>
                </div>
                `)
...
function fidgetSpinner(page) {
    ...
    if (page == '.missing-page-container') {
        $.post('https://Nmsh-mdt/getAllMissing', JSON.stringify({}));
    }
    if (page == '.reports-page-container') {
        $.post('https://Nmsh-mdt/getAllReports', JSON.stringify({}));
    }
    if (page == '.stafflogs-page-container') {
        $.post('https://Nmsh-mdt/getAllLogs', JSON.stringify({}));
    }
...
function timeShit() {
    ...
    date = localDate.toLocaleDateString('en-US', { timeZone: 'Asia/Riyadh' });
    time = localDate.toLocaleTimeString('en-US', { timeZone: 'Asia/Riyadh' });
    $(".date").html(date)
    $(".time").html(time)
}
...
function addTag(tagInput) {
    ...
    $.post('https://Nmsh-mdt/newTag', JSON.stringify({
...
function addWeapon(Input) {
    ...
    $.post('https://Nmsh-mdt/newWeapon', JSON.stringify({
        id: $(".manage-profile-citizenid-input").val(),
        serialnumber: Input
    }));
}
...
function removeImage(url) {
    ...
    $.post('https://Nmsh-mdt/removeGalleryImg', JSON.stringify({
        cid: cid,
        URL: url
    }));
}
