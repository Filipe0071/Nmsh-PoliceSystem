...
LoadQBCoreVersion = function()
    Citizen.CreateThread(function()
        while true do
            ...
            if dist < 2 then
                DrawText3D(Config["PublicRecords"]['Coords'], "[~f~E~w~] View Public Records")
    ...
    OpenPublicRecords = function()
        ...
        TriggerEvent('Nmsh-mdt:animation')
        local PlayerInfo = RPC.execute("Nmsh-mdt:getInfo")
    ...
    function EnableGUI(enable)
        if enable then 
            SendNUIMessage({type = "bulletin", data = RPC.execute("Nmsh-mdt:dashboardbulletin")})
            SendNUIMessage({type = "dispatchmessages", data = RPC.execute("Nmsh-mdt:dashboardMessages")})
            SendNUIMessage({type = "getActiveUnits", lspd = RPC.execute("Nmsh-mdt:getActiveLSPD"), ems = RPC.execute("Nmsh-mdt:getActiveEMS")})
            ...
            SendNUIMessage({type = "warrants", data = RPC.execute("Nmsh-mdt:getWarrants")})
        end
        SetNuiFocus(enable, enable)
        SendNUIMessage({type = "show", enable = enable, job = QBCore.Functions.GetPlayerData().job.name, isAdmin = RPC.execute("Nmsh-mdt:isAdmin")})
        isOpen = enable
        TriggerEvent('Nmsh-mdt:animation')
    end
    function RefreshGUI()
        SetNuiFocus(false, false)
        SendNUIMessage({type = "show", enable = false, job = QBCore.Functions.GetPlayerData().job.name, isAdmin = RPC.execute("Nmsh-mdt:isAdmin")})
        isOpen = false
    end
    AddEventHandler('Nmsh-mdt:animation', function()
        if not isOpen then 
            return 
        end
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("Nmsh-mdt:deleteBulletin", data.id, data.time)
    ...
    RegisterNUICallback("updateLicence", function(data, cb)
        ...
        local cid = data.cid
        RPC.execute("Nmsh-mdt:updateLicence", type, status, cid)
    end)
    RegisterNUICallback("newBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("Nmsh-mdt:newBulletin", data.title, data.info, data.time)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
        if job == 'police' and QBCore.Functions.GetPlayerData().job.name == "police" then
    ...
    RegisterNetEvent('Nmsh-mdt:open')
    AddEventHandler('Nmsh-mdt:open', function(job, jobLabel, lastname, firstname)
        open = true
        EnableGUI(open)
        local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))
        local currentStreetHash, intersectStreetHash = GetStreetNameAtCoord(x, y, z)
    ...
    RegisterNUICallback("searchProfiles", function(data, cb)
        SendNUIMessage({type = "profiles", data = RPC.execute("Nmsh-mdt:searchProfile", data.name), isLimited = RPC.execute("Nmsh-mdt:isLimited")})
    end)
    RegisterNUICallback("saveProfile", function(data, cb)
        RPC.execute("Nmsh-mdt:saveProfile", data.pfp, data.description, data.id)
    end)
    RegisterNUICallback("newWeapon", function(data, cb)
        RPC.execute("Nmsh-mdt:addWeapon", data.id, data.serialnumber)
    end)
    RegisterNUICallback("getProfileData", function(data, cb)
        local isLimited = RPC.execute("Nmsh-mdt:isLimited")
        local ProfileData = RPC.execute("Nmsh-mdt:getProfileData", data.id)
    ...
    RegisterNUICallback("newTag", function(data, cb)
        if data.id ~= "" and data.tag ~= "" then
            RPC.execute("Nmsh-mdt:newTag", data.id, data.tag)
        end
    end)
    RegisterNUICallback("removeProfileTag", function(data, cb)
        RPC.execute("Nmsh-mdt:removeProfileTag", data.cid, data.text)
    end)
    RegisterNUICallback("addGalleryImg", function(data, cb)
        RPC.execute("Nmsh-mdt:addGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("missingCitizen", function(data, cb)
        RPC.execute("Nmsh-mdt:missingCitizen", data.cid, data.bool)
    end)
    RegisterNUICallback("removeGalleryImg", function(data, cb)
        RPC.execute("Nmsh-mdt:removeGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("searchIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("Nmsh-mdt:searchIncidents", data.incident)})
    end)
    RegisterNUICallback("getIncidentData", function(data, cb)
        local sentData, sentConvictions = RPC.execute("Nmsh-mdt:getIncidentData", data.id)
    ...
    RegisterNUICallback("incidentSearchPerson", function(data, cb)
        SendNUIMessage({type = "incidentSearchPerson", data = RPC.execute("Nmsh-mdt:incidentSearchPerson", data.name)})
    end)
    RegisterNUICallback("searchBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("Nmsh-mdt:searchBolos", data.searchVal)})
    end)
    RegisterNUICallback("getAllBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("Nmsh-mdt:getAllBolos")})
    end)
    RegisterNUICallback("getAllMissing", function(data, cb)
        SendNUIMessage({type = "missing", data = RPC.execute("Nmsh-mdt:getAllMissing")})
    end)
    RegisterNUICallback("getAllIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("Nmsh-mdt:getAllIncidents")})
    end)
    RegisterNUICallback("getBoloData", function(data, cb)
        SendNUIMessage({type = "boloData", data = RPC.execute("Nmsh-mdt:getBoloData", data.id)})
    end)
    RegisterNUICallback("jailPlayer", function(data, cb)
        RPC.execute("Nmsh-mdt:JailPlayer", data.cid, data.time)
    end)
    RegisterNUICallback("newBolo", function(data, cb)
        local sentData = RPC.execute("Nmsh-mdt:newBolo", data.existing, data.id, data.title, data.plate, data.owner, data.individual, data.detail, data.tags, data.gallery, data.officers, data.time)
        SendNUIMessage({type = "boloComplete", data = sentData})
    end)
    RegisterNUICallback("deleteBolo", function(data, cb)
        RPC.execute("Nmsh-mdt:deleteBolo", data.id, data.time)
    end)
    RegisterNUICallback("deleteIncident", function(data, cb)
        RPC.execute("Nmsh-mdt:deleteIncident", data.id, data.time)
    end)
    RegisterNUICallback("deleteMissing", function(data, cb)
        RPC.execute("Nmsh-mdt:deleteMissing", data.id, data.time)
    end)
    RegisterNUICallback("deleteReport", function(data, cb)
        RPC.execute("Nmsh-mdt:deleteReport", data.id, data.time)
    end)
    RegisterNUICallback("deleteICU", function(data, cb)
        RPC.execute("Nmsh-mdt:deleteICU", data.id)
    end)
    RegisterNUICallback("getAllReports", function(data, cb)
        SendNUIMessage({type = "reports", data = RPC.execute("Nmsh-mdt:getAllReports")})
    end)
    RegisterNUICallback("getReportData", function(data, cb)
        SendNUIMessage({type = "reportData", data = RPC.execute("Nmsh-mdt:getReportData", data.id)})
    end)
    RegisterNUICallback("searchReports", function(data, cb)
        SendNUIMessage({type = "reports", data = RPC.execute("Nmsh-mdt:searchReports", data.name)})
    end)
    RegisterNUICallback("newReport", function(data, cb)
        local sentData = RPC.execute("Nmsh-mdt:newReport", data.existing, data.id, data.title, data.type, data.detail, data.tags, data.gallery, data.officers, data.civilians, data.time)
        SendNUIMessage({type = "reportComplete", data = sentData})
    end)
    RegisterNUICallback("searchVehicles", function(data, cb)
        local sentData = RPC.execute("Nmsh-mdt:searchVehicles", data.name)
        for i = 1, #sentData do
    ...
    RegisterNUICallback("saveVehicleInfo", function(data, cb)
        RPC.execute("Nmsh-mdt:saveVehicleInfo", data.plate, data.imageurl, data.notes)
    end)
    RegisterNUICallback("saveWeaponInfo", function(data, cb)
        RPC.execute("Nmsh-mdt:saveWeaponInfo", data.serialnumber, data.imageurl, data.brand, data.type, data.notes)
    end)
    RegisterNUICallback("saveMissingInfo", function(data, cb)
        RPC.execute("Nmsh-mdt:saveMissingInfo", data.id, data.imageurl, data.notes)
    end)
    RegisterNUICallback("knownInformation", function(data, cb)
        RPC.execute("Nmsh-mdt:knownInformation", data.type, data.status, data.plate)
    end)
    RegisterNUICallback("getAllLogs", function(data, cb)
        SendNUIMessage({type = "getAllLogs", data = RPC.execute("Nmsh-mdt:getAllLogs")})
    end)
    RegisterNUICallback("getPenalCode", function(data, cb)
        local titles, penalcode, job = RPC.execute("Nmsh-mdt:getPenalCode")
        SendNUIMessage({type = "getPenalCode", titles = titles, penalcode = penalcode, job = job})
    end)
    RegisterNUICallback("toggleDuty", function(data, cb)
        RPC.execute("Nmsh-mdt:toggleDuty", data.cid, data.status)
    end)
    RegisterNUICallback("setCallsign", function(data, cb)
        callSign = RPC.execute("Nmsh-mdt:setCallsign", data.cid, data.newcallsign)
    end)
    RegisterNUICallback("saveIncident", function(data, cb)
        RPC.execute("Nmsh-mdt:saveIncident", data.ID, data.title, data.information, data.tags, data.officers, data.civilians, data.evidence, data.associated, data.time)
    end)
    RegisterNUICallback("removeIncidentCriminal", function(data, cb)
        RPC.execute("Nmsh-mdt:removeIncidentCriminal", data.cid, data.incidentId)
    end)
    RegisterNUICallback("setWaypoint", function(data, cb)
        local callInformation = RPC.execute("Nmsh-mdt:setWaypoint", data.callid)
        SetNewWaypoint(callInformation['origin']['x'], callInformation['origin']['y'])
    end)
    RegisterNUICallback("callDetach", function(data, cb)
        local callid, sentData = RPC.execute("Nmsh-mdt:callDetach", data.callid)
    ...
    RegisterNUICallback("removeCall", function(data, cb)
        local src, callid = RPC.execute("Nmsh-mdt:removeCall", data.callid)
    ...
    RegisterNUICallback("callAttach", function(data, cb)
        local callid, sentData = RPC.execute("Nmsh-mdt:callAttach", data.callid)
        local job = QBCore.Functions.GetPlayerData().job.name
        if job == "police" or job == 'ambulance' then
    ...
    RegisterNUICallback("attachedUnits", function(data, cb)
        local sentData, callid = RPC.execute("Nmsh-mdt:attachedUnits", data.callid)
        SendNUIMessage({type = "attachedUnits", data = sentData, callid = callid})
    end)
    RegisterNUICallback("callDispatchDetach", function(data, cb)
        RPC.execute("Nmsh-mdt:callDispatchDetach", data.callid, data.cid)
    end)
    RegisterNUICallback("setDispatchWaypoint", function(data, cb)
        local callInformation = RPC.execute("Nmsh-mdt:setDispatchWaypoint", data.callid)
        SetNewWaypoint(callInformation['origin']['x'], callInformation['origin']['y'])
    end)
    RegisterNUICallback("callDragAttach", function(data, cb)
        RPC.execute("Nmsh-mdt:callDragAttach", data.callid, data.cid)
    end)
    RegisterNUICallback("setWaypointU", function(data, cb)
        local sentData = RPC.execute("Nmsh-mdt:setWaypoint:unit", data.cid)
        SetNewWaypoint(sentData.x, sentData.y) 
    end)
    RegisterNUICallback("dispatchMessage", function(data, cb)
        local sentData = RPC.execute("Nmsh-mdt:sendMessage", data.message, data.time)
        local job = QBCore.Functions.GetPlayerData().job.name
        if job == "police" or job == 'ambulance' then
    ...
    RegisterNUICallback("refreshDispatchMsgs", function(data, cb)
        SendNUIMessage({type = "dispatchmessages", data = RPC.execute("Nmsh-mdt:refreshDispatchMsgs")})
    end)
end
...
LoadESXVersion = function()
    ...
    function EnableGUI(enable)
        ...
        SetNuiFocus(enable, enable)
        SendNUIMessage({type = "show", enable = enable, job = ESX.GetPlayerData().job.name, isAdmin = RPC.execute("Nmsh-mdt:isAdmin")})
        isOpen = enable
        TriggerEvent('Nmsh-mdt:animation')
    end
    function RefreshGUI()
        SetNuiFocus(false, false)
        SendNUIMessage({type = "show", enable = false, job = ESX.GetPlayerData().job.name, isAdmin = RPC.execute("Nmsh-mdt:isAdmin")})
        isOpen = false
    end
    AddEventHandler('Nmsh-mdt:animation', function()
        if not isOpen then 
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("Nmsh-mdt:deleteBulletin", data.id)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
    ...
    RegisterNUICallback("deleteBolo", function(data, cb)
        RPC.execute("Nmsh-mdt:deleteBolo", data.id)
    end)
    RegisterNUICallback("deleteIncident", function(data, cb)
        RPC.execute("Nmsh-mdt:deleteIncident", data.id, data.time)
    end)
...
LoadNopixelVersion = function()
    ...
    function EnableGUI(enable)
        ...
        SetNuiFocus(enable, enable)
        SendNUIMessage({type = "show", enable = enable, job = exports['iconic_handler']:isPed("myjob"), isAdmin = RPC.execute("Nmsh-mdt:isAdmin")})
        isOpen = enable
        TriggerEvent('Nmsh-mdt:animation')
    end
    function RefreshGUI()
        SetNuiFocus(false, false)
        SendNUIMessage({type = "show", enable = false, job = exports['iconic_handler']:isPed("myjob"), isAdmin = RPC.execute("Nmsh-mdt:isAdmin")})
        isOpen = false
    end
    AddEventHandler('Nmsh-mdt:animation', function()
        if not isOpen then 
